...
command_parser = argparse.ArgumentParser(
    description="Generate smokers dataset where relations follow a grid model"
)
...
program_prolog_tabled_smokers_grid = """
...
for i in range(0, arguments.replicas):
    # here the replicas only differ in the node for the query, the program is the same
    ...
    for t in ["tabled", "original"]:
        filename = f"smokers_grid_{t}_n_{arguments.n}_{i}.pl"
        ...
        if t == "tabled":
            ...
            fp.write(program_prolog_tabled_smokers_grid)
        else:
            filenames_aspmc.append(filename)
            fp.write(program_aspmc_smokers_grid)
   
        # write stress/1 facts
        fp.write(f"% nodes: {len(nodes)}\n")
        for node in nodes:
            fp.write(f"{default_probability_stress}::stress({node}).\n")

        # write edges/2 facts
        fp.write(f"% edges: {len(graph.edges)}\n")
        for el in graph.edges:
            fp.write(f"{default_probability_edge}::e(n{el[0][0]}{el[0][1]},n{el[1][0]}{el[1][1]}).\n")
