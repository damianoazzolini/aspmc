...
for sz in range(1, 7):
    for alg in ["COBYLA","SLSQP"]:
        for eps in [0,0.06]:
            ...
            fname = f"runner_smoke_{sz}_{alg}_{s}.sh"
            fp = open(fname, "w")
...
#SBATCH --output=smoke_{sz}_{alg}_{s}.log

echo "Started at: "
date

time pasta t{sz}.lp --query="qr" --optimize {"--epsilon=0.06" if eps == 0.06 else ""} --threshold=0.01 --target=upper --verbose --method={alg}
