...
def printer(l_in : 'list[str]', filename : str, method : str):
    ...
    preamble = "#!/bin/bash\n" + \
               f"#SBATCH --job-name=qr_{m}_cn{len(l_in)}_c\n" + \
               "#SBATCH --ntasks=1\n" + \
               "#SBATCH --mem=16gb\n" + \
               "#SBATCH --partition=longrun\n" + \
               f"#SBATCH --output=qr_nqr_clauses_not_{method}_{len(l_in)}_constr.log\n\n"
    fp.write(preamble)
    
    preamble = "#!/bin/bash\n" + \
            f"#SBATCH --job-name=qr_{m}_cn{len(l_in)}_noc\n" + \
            "#SBATCH --ntasks=1\n" + \
            "#SBATCH --mem=16gb\n" + \
            "#SBATCH --partition=longrun\n" + \
            f"#SBATCH --output=qr_nqr_clauses_not_{method}_{len(l_in)}_no_constr.log\n\n"
